...
namespace ProcessingPhoto
{
    ...
    class Morphology : Filtres
    {
        ...
        {
            int radX = st_elem.GetLength(0) / 2;//наш структурный элемент 3х3, радиус по одной клетке
    ...
    class Opening : Morphology // Морфологическое открытие
    {
        ...
        public override Bitmap processImage(Bitmap sourceImage, BackgroundWorker worker)
        {
            ...
            Bitmap resultImage = new Bitmap(sourceImage.Width, sourceImage.Height);
            Filtres Err = new Erosion(st_elem);
            Filtres Dila = new Dilation(st_elem);
